    ...
    public class FeedBackMapper :Profile
    {
        public FeedBackMapper()
        {
            ...
            CreateMap<FeedBack, FeedBackGetDto>()
                .ForMember(x => x.SenderId , opt=>opt.MapFrom(src => src.IsAnonymous ? 0 :src.SenderId))
                .ForMember(x => x.ReceiverId , opt=>opt.MapFrom(src=>src.ReceiverId))
                .ForMember(x=>x.SenderUserName,opt=>opt.MapFrom(src=>src.IsAnonymous? "Anonymous": src.Sender.UserName))
